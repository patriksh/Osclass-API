...
class ItemActions
{
  ...
  {
    $aItem     = $this->data;
    $aItem     = osc_apply_filter('api_item_add_prepare_data', $aItem);
    $is_spam   = 0;
    $enabled   = 1;
    ...
    osc_run_hook('api_pre_item_post');
    osc_run_hook('api_pre_item_add', $aItem, $flash_error);
    $flash_error = osc_apply_filter('api_pre_item_add_error', $flash_error, $aItem);
    ...
    if ($flash_error) {
      ...
      osc_run_hook('api_posted_item', $item);
    }
      ...
      foreach ($aResources[ 'error' ] as $key => $error) {
        if ($maxImagesPerItem == 0 || ($maxImagesPerItem > 0 && $totalItemImages < $maxImagesPerItem)) {
          if ($error == UPLOAD_ERR_OK) {
            ...
            osc_run_hook('api_uploaded_file', ItemResource::newInstance()->findByPrimaryKey($resourceId));
          }
        }
      }
  ...
  private function _increaseStats($item)
  {
    ...
    osc_run_hook('api_item_increase_stat', $item);
  }
  ...
  {
    $aItem     = $this->data;
    $aItem     = osc_apply_filter('api_item_edit_prepare_data', $aItem);
    $flash_error = '';
    ...
    osc_run_hook('api_pre_item_post');
    osc_run_hook('api_pre_item_edit', $aItem, $flash_error);
    $flash_error = osc_apply_filter('api_pre_item_edit_error', $flash_error, $aItem);
    ...
    if ($flash_error) {
      ...
      // THIS HOOK IS FINE, YAY!
      osc_run_hook('api_edited_item', Item::newInstance()->findByPrimaryKey($aItem[ 'idItem' ]));
      $success = $result;
    }
  ...
  public function activate($id, $secret = null)
  {
    ...
    if (isset($item[0]['b_enabled'], $item[0]['b_active']) && $item[0]['b_enabled'] == 1 && $item[0]['b_active'] == 0) {
      ...
      // updated correctly
      if ($result == 1) {
        osc_run_hook('api_activate_item', $id);
    ...
    if (isset($item[0]['b_enabled']) && $item[0]['b_enabled'] == 1) {
      if(osc_isExpired($item[0]['dt_expiration'])) {
        if((osc_renewal_limit() > 0 && (int)$item[0]['i_renewed'] < osc_renewal_limit()) || osc_renewal_limit() <= 0) {
          ...
          // updated correctly
          if ($result == 1) {
            osc_run_hook('api_renew_item', $id);
            // b_enabled == 1 && b_active == 1
  ...
  public function deactivate_by_id($id)
  {
    ...
    // updated correctly
    if ($result == 1) {
      osc_run_hook('api_deactivate_item', $id);
  ...
  private function _decreaseStats($item)
  {
    ...
    osc_run_hook('api_item_decrease_stat', $item);
  }
  ...
  public function enable($id)
  {
    ...
    // updated correctly
    if ($result == 1) {
      osc_run_hook('api_enable_item', $id);
      $item = $this->manager->findByPrimaryKey($id);
  ...
  public function disable($id)
  {
    ...
    // updated correctly
    if ($result == 1) {
      osc_run_hook('api_disable_item', $id);
      $item = $this->manager->findByPrimaryKey($id);
      if ($item[ 'b_active' ] == 1 && $item[ 'b_spam' ] == 0 && !osc_isExpired($item[ 'dt_expiration' ])) {
        $this->_decreaseStats($item);
      }
  ...
  public function premium($id, $on = true)
  {
    ...
    if ($result == 1) {
      if ($on) {
        osc_run_hook('api_item_premium_on', $id);
      } else {
        osc_run_hook('api_item_premium_off', $id);
      }
  ...
  public function spam($id, $on = true)
  {
    ...
    if ($result == 1) {
      if ($on) {
        osc_run_hook('api_item_spam_on', $id);
      } else {
        osc_run_hook('api_item_spam_off', $id);
      }
  ...
  public function delete($secret, $itemId)
  {
    ...
    osc_run_hook('api_before_delete_item', $itemId);
    ...
    if ($item[ 's_secret' ] == $secret) {
      ...
      if ($result !== false) {
        osc_run_hook('api_after_delete_item', $itemId, $item);
      }
  ...
  public function add_comment()
  {
    ...
    osc_run_hook('api_before_add_comment', $aComment);
    ...
    if ($mComments->insert($aComment)) {
      ...
      osc_run_hook('api_add_comment', $commentID);
  ...
  public function prepareData($is_add)
  {
    ...
    $aItem    = osc_apply_filter('api_item_prepare_data', $aItem);
    $this->data = $aItem;
  }
}
